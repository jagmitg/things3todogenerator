...
const ToDoGenerator = () => {
    ...
    useEffect(() => {
        const loadConfig = async () => {
            try {
                ...
                setIncludeAdditional(config.options.map((option) => ({ ...option, checked: false })));
            } catch (error) {
    ...
    const handleGenerateLink = async () => {
        ...
        try {
            ...
            if (!baseSections) {
                throw new Error("Base sections not found");
            }
            ...
            // Save generated link to local storage under a specific key
            const generatedLinks = JSON.parse(localStorage.getItem("generatedLinks") || "[]");
            generatedLinks.push({ title, category: selectedList, link: url });
    ...
    const handleClear = () => {
        ...
        setGeneratedLink("");
        setError("");
        setIncludeAdditional(includeAdditional.map((option) => ({ ...option, checked: false })));
    };
    ...
    const handleListChange = async (e) => {
        ...
        if (listCategory) {
            const filteredOpts = options.filter((option) => option.category === listCategory);
        ...
        <div className="container mx-auto mt-10 px-4">
            <div className="bg-white shadow-md rounded-lg p-6">
                ...
                <form>
                    ...
                    <div className="mb-4">
                        <label htmlFor="formList" className="block text-gray-700 font-bold mb-2">
